...
public class SqlFormatter {
	...
	public String format(String source, boolean removeSpace) {
		return new FormatProcess(source,removeSpace).perform();
	}
...
//		}
		...
		public FormatProcess(String sql, boolean removeSpace) {
		...
		public String perform() {
			...
			while (tokens.hasMoreTokens()) {
				...
				if (afterByOrSetOrFromOrSelect && ",".equals(token)) {
					...
					// 如果上一个token非空或者不需要删除多余空格时打印空白字符
					if (!lastSpaceToken || !removeSpace) {
						white();
					}
					lastSpaceToken = true;
				}
